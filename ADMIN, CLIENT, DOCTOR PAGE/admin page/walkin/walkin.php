...
<?php } ?>
    ...
    // Fetch data from walkins table
    $query_walkins = 'SELECT * FROM walkins';
    $result_walkins = mysqli_query($con, $query_walkins);

    // Fetch data from doctor_walkin table
    $query_doctor_walkin = 'SELECT * FROM doctor_walkin';
    $result_doctor_walkin = mysqli_query($con, $query_doctor_walkin);

    // Check if both queries were successful
    if ($result_walkins && $result_doctor_walkin) {
        $counter = 1;

        // Fetch and display data from walkins table
        while ($walkins = mysqli_fetch_assoc($result_walkins)) {
            ?>
            <tr>
                <td><?= $counter ?></td>
                ...
                <td>
                    <a href="../walkin_history1/walkin_history.php?patientID=<?= $walkins['id']; ?>" class="btn btn-info">View</a>
 ...
 <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#deleteSchedModal<?= $counter - 1 ?>">Delete</button>
        ...
        // Fetch and display data from doctor_walkin table
        while ($doctor_walkin = mysqli_fetch_assoc($result_doctor_walkin)) {
            ?>
            <tr>
                <td><?= $counter ?></td>
                <td><?= $doctor_walkin['firstname'] . ' ' . $doctor_walkin['middlename'] . ' ' . $doctor_walkin['lastname'] ?></td>
                <td><?= $doctor_walkin['email'] ?></td>
                <td><?= $doctor_walkin['address'] ?></td>
                <td><?= $doctor_walkin['gender'] ?></td>
                <td>
                <a href="../walkin_history/walkin_history.php?patientID=<?= $doctor_walkin['id']; ?>" class="btn btn-info">View</a>
                    <button type="button" value="<?= $doctor_walkin['id'] ?>" class="editwalkinsBtn btn btn-warning">Edit</button>
